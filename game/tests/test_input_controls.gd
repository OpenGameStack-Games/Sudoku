...
func test_layout_and_styling() -> void:
	...
	if checked_icon:
		assert_eq(checked_icon.get_size(), Vector2(56, 28), "Checked icon should be 56x28")
		
	var unchecked_icon: Texture2D = controls.auto_candidate_btn.get_theme_icon("unchecked")
	assert_true(unchecked_icon != null, "Auto candidate button should have unchecked icon")
	if unchecked_icon:
		assert_eq(unchecked_icon.get_size(), Vector2(56, 28), "Unchecked icon should be 56x28")
