...
- **Single Issue Isolation (Anti-Batching):** You (the orchestrator) must NEVER instruct an `issue_resolver` to resolve multiple issues in a single prompt (e.g., "Resolve issues 47, 48, and 49"). Batching overloads the agent's context window and causes hallucinations. You must spawn a dedicated `issue_resolver` subagent for EXACTLY ONE issue, wait for it to finish, invoke the `pr_reviewer` to merge it, and then spawn a BRAND NEW `issue_resolver` subagent for the next issue.
