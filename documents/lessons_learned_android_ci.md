...
## 15. Automating Google Play Store Deployment (Direct Production Track)
Deploying signed release Android App Bundles directly to the Google Play Store via GitHub Actions introduces specific requirements around API access, release tracks, and artifact diagnostics:
- **Direct-to-Production Track Prerequisites:** Google Play Developer API allows automated deployment to the `production` track (`track: production` with `status: completed`), provided that the app (`games.audrain.sudoku`) has already undergone initial manual review and publication through the Google Play Console. Subsequent releases can bypass testing tracks (internal, alpha, beta) entirely for seamless continuous delivery.
- **Service Account Authentication:** Automated uploading uses `r0adkll/upload-google-play@v1` authenticated via a dedicated Google Cloud Service Account JSON key (`GOOGLE_PLAY_SERVICE_ACCOUNT_JSON`). The service account must be invited in the Google Play Console under **Users and Permissions** with "Release apps to production, exclude devices, and use Play App Signing" and "Manage testing tracks" permissions.
- **Complete Diagnostic Artifacts:** Google Play Console flags warnings and limits crash symbolication if App Bundles are uploaded without native debug symbols or obfuscation mapping files. The workflow uploads the bundle (`releaseFiles: ./game/*.aab`), ProGuard mapping file (`mappingFile: ./game/android/build/outputs/mapping/release/mapping.txt`), and native debug symbols (`debugSymbols: ./game/*-native-debug-symbols.zip`) simultaneously.
- **Pipeline Order and Tag Guarding:** Deployment MUST strictly execute after keystore signing (`r0adkll/sign-android-release@v1`) so that debug or unsigned builds are never sent to Google Play. Furthermore, deployment is guarded with `if: startsWith(github.ref, 'refs/tags/v')` so that pull request checks and branch pushes build artifacts without attempting unauthenticated or unwanted store publication.
