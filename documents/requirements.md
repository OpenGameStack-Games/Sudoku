...
- **Statistics Screen (`game/scenes/statistics_screen.tscn` & `game/scripts/statistics_screen.gd`):** A dedicated screen to display the player's historical performance.
  ...
  - **1930s Styling & Layout:** Built over a Dark Gray background (`#121212`, `ThemeConstants.COLOR_BG_DARK_GRAY`) featuring 1930s monochrome panel cards (2px white borders, 8px rounded corners, and generous 40px content margin padding on all sides) wrapped inside a responsive `ScrollContainer` ensuring seamless layout adaptation across small mobile phones and large tablets. The cards utilize enlarged vertical scaling and element spacing (30px card VBox separation, 30px horizontal and 20px vertical grid separation) to effectively utilize screen real estate while maintaining aesthetic balance.
  - **Difficulty Breakdown:** Independent styled cards for Easy, Medium, and Hard difficulties with enlarged difficulty headers (font size 48) and stat items (font size 32) displaying:
