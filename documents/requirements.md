...
- **Statistics Screen (`game/scenes/statistics_screen.tscn` & `game/scripts/statistics_screen.gd`):** A dedicated screen to display the player's historical performance.
  ...
  - **1930s Styling & Layout:** Built over a Dark Gray background (`#121212`, `ThemeConstants.COLOR_BG_DARK_GRAY`) featuring 1930s monochrome panel cards (2px white borders, 8px rounded corners, and generous content margin padding on all sides). The layout relies on a fully dynamically scaling `VBoxContainer` setup (removing `ScrollContainer` entirely) combined with a custom `_scale_ui()` script. This dynamically scales text `font_size` and margins based on the viewport height, ensuring all three difficulty cards fit simultaneously and adapt seamlessly across small mobile phones and large tablets without requiring scroll boundaries.
...
- **Gameplay Screen (`game/scenes/gameplay_screen.tscn` & `game/scripts/gameplay_screen.gd`):** The primary gameplay scene coordinating the header controls, 9x9 board, input controls, and pause overlay:
  ...
  - **Sudoku Grid (`game/scenes/board.tscn` & `game/scenes/cell.tscn`):** A standard 9x9 grid, visually sectioned into 3x3 macro blocks. Each cell contains a 3x3 candidate micro-grid.
    ...
    - **Positional Candidate 3x3 Micro-Grid:** Candidate numbers inside each cell are arranged in a permanently fixed 3x3 phone-numpad positional layout (`GridContainer` with 3 columns):
      ...
      To prevent Godot's `GridContainer` from reflowing child elements when candidates are absent, all 9 candidate `Label` nodes remain permanently visible (`visible = true`) in the layout hierarchy, featuring an explicit 20x20 minimum size lock (`custom_minimum_size = Vector2(20, 20)`) and text clipping (`clip_text = true`). This prevents the bottom row from overflowing the cell bounds and ensures layout stability even when active candidates scale to a larger font size. Active candidates set their `text` property to the corresponding digit string, while inactive candidates set their `text` to an empty string (`""`), preserving rigid positional alignment across all combinations of active notes.
