...
- **Theme and Aesthetics:** The game must use a 1930s monochrome cartoon/animation style (e.g., Steamboat Willie). The color palette is specifically **white-on-black**.
  ...
  - **1930s Theme Resource (`game/resources/theme_1930s.tres`):** Pre-configured Godot `Theme` resource establishing:
    ...
    - Monochrome symbol enforcement: UI buttons utilizing Unicode symbols (e.g. Pause, Resume `▶︎`, Undo `↺`, Redo `↻`) MUST rely on the `SystemFont` fallback sequence embedded in `theme_1930s.tres` (prioritizing monochrome symbol fonts) to ensure they render as white shapes, as solely appending the text variation selector (`U+FE0E`) fails on certain OS fallback stacks.
    - Cross-Platform & Web Export Symbol Font Fallback (`game/assets/fonts/NotoSansSymbols-Regular.ttf`): Bundled open-source font asset licensed under SIL Open Font License providing complete glyph coverage for `↺` (U+21BA), `↻` (U+21BB), and `▶` (U+25B6). Configured as an explicit fallback (`fallbacks` array) on `SystemFont_button` in `theme_1930s.tres`. In addition, `theme_1930s.tres` explicitly sets `Label/fonts/font = SubResource("SystemFont_button")` so child label nodes (such as `UndoBtn/Icon` and `RedoBtn/Icon`) inherit the fallback sequence, guaranteeing consistent rendering in sandboxed browser web exports (HTML5/WebAssembly) and mobile builds without missing glyph boxes.
  ...
  - **Automated Verification:** Validated via unit tests in `game/tests/test_theme_constants.gd` (asserting theme resource existence, asset file existence for `NotoSansSymbols-Regular.ttf`, default button and label font configurations, background hex color fidelity, constant distinctness, and panel stylebox properties) and `game/tests/test_input_controls.gd` (`test_undo_redo_icons()` asserting label existence and glyph integrity).
...
- **Main Menu Screen (`game/scenes/main_menu.tscn` & `game/scripts/main_menu.gd`):** The primary entry point of the game (instanced by `game/scenes/main.tscn` as configured in `project.godot`). Embodies the 1930s monochrome cartoon aesthetic:
  ...
  - **Automated Verification:** Verified in headless CI via `game/tests/test_main_menu.gd`, asserting scene and asset existence (`mascot_icon.jpg`, `main_menu.tscn`, `main.tscn`), default vs resumed button labels, signal routing, root scene initialization, and credits menu layout elements.
- **Statistics Screen (`game/scenes/statistics_screen.tscn` & `game/scripts/statistics_screen.gd`):** A dedicated screen to display the player's historical performance.
  ...
  - **Automated Verification:** Verified in headless CI via `game/tests/test_statistics_screen.gd` (`godot --headless --path game -s res://tests/test_runner.gd`), asserting scene and script asset presence, mock stats data binding across all three difficulty tiers, fallback empty states, enlarged typography and padding styling assertions (40px margin, 48pt difficulty header, 32pt grid text), and back navigation button signal wiring.
- **Gameplay Screen (`game/scenes/gameplay_screen.tscn` & `game/scripts/gameplay_screen.gd`):** The primary gameplay scene coordinating the header controls, 9x9 board, input controls, and pause overlay:
  ...
  - **Input Controls & Numpad Component (`game/scenes/input_controls.tscn` & `game/scripts/input_controls.gd`):** A standalone UI component managing player input below the 9x9 board, fully conforming to the 1930s monochrome styling via `res://resources/theme_1930s.tres`:
    ...
    - **Mode & Action Row:**
      ...
      - Right side (spaced apart): "Undo" button (labeled '↺', with the icon explicitly rotated -90 degrees to point outward left, minimum size `Vector2(80, 80)`, 32pt font) wired to `board.undo_manager.undo_last_action(board)`, automatically disabled when the undo history stack is empty. A "Redo" button (labeled '↻', with the icon explicitly rotated 90 degrees to point outward right, minimum size `Vector2(80, 80)`, 32pt font) wired to `board.undo_manager.redo_last_action(board)`, automatically disabled when the redo history stack is empty. Note: The Redo history stack is flushed whenever a new board action is taken.
    ...
    - **Numpad Row:**
      ...
      - **Button Sizing & Font Specification:** All numpad buttons configure a minimum vertical size of 64px (`custom_minimum_size = Vector2(0, 64)`) with horizontal expansion (`size_flags_horizontal = 3`), 8px grid separation (`h_separation = 8`, `v_separation = 8`), and explicit 36px font sizing (`theme_override_font_sizes/font_size = 36`) for legibility. This gives the buttons a taller, more balanced square or portrait aspect ratio on mobile viewports.
...
- **Automated Verification:** Automated unit tests in `game/tests/test_platform_config.gd` validate that `export_presets.cfg` disables immersive mode, preserves the `*.json` include filter, ensures mascot icon assets exist, and verifies orientation, stretch project settings, and splash screen configurations.
