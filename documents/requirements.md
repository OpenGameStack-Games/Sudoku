...
- **Main Menu Screen (`game/scenes/main_menu.tscn` & `game/scripts/main_menu.gd`):** The primary entry point of the game (instanced by `game/scenes/main.tscn` as configured in `project.godot`). Embodies the 1930s monochrome cartoon aesthetic:
  ...
  - **Credits Menu & Attributions:** A "Credits" button positioned on the Main Menu that opens a full-screen `CreditsModal`. The modal must contain studio attributions and external web links for Open Game Stack, Audrain Entertainment, and the open-source GitHub repository. The URLs function via `OS.shell_open()` to launch the system browser.
...
- **Statistics Screen (`game/scenes/statistics_screen.tscn` & `game/scripts/statistics_screen.gd`):** A dedicated screen to display the player's historical performance.
  ...
  - **1930s Styling & Layout:** Built over a Dark Gray background (`#121212`, `ThemeConstants.COLOR_BG_DARK_GRAY`) featuring 1930s monochrome panel cards (2px white borders, 8px rounded corners, and generous content margin padding on all sides). The layout relies on a `ScrollContainer` to allow native vertical scrolling, ensuring that all content and difficulty cards are accessible and do not clip on extreme aspect ratios (such as mobile portrait mode) rather than forcing a tight responsive scale.
