...
- **Input Controls & Numpad (`game/scenes/input_controls.tscn` & `game/scripts/input_controls.gd`):** Standalone 1930s-styled control panel featuring Normal/Candidate mode toggles, dynamically synchronized Undo button (disabled on empty history, enabled upon moves), 1-9 & Erase (X) numpad with cheat-proof digit exhaustion dimming (`ThemeConstants.COLOR_NUMPAD_EXHAUSTED`), Auto Candidate toggle, and full bi-directional (Cell-First and Number-First) input routing.
