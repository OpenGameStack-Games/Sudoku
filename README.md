...
- **SaveManager (`game/autoloads/save_manager.gd`):** Global autoload managing multi-difficulty save state persistence across Very Easy, Easy, Medium, Hard, and Very Hard slots (`user://saves/save_<difficulty>.json`). Automatically flushes complete game state (puzzle string, cell values, candidate notes, user deleted candidates, undo history stack, and elapsed seconds) upon state transitions (board updates, undo actions, window/app focus loss), with robust fallback and recovery from corrupted save files.
- **Board & Cell UI Components (`game/scenes/board.tscn`, `game/scenes/cell.tscn`, `game/scripts/board_ui.gd`, `game/scripts/cell_ui.gd`):** Interactive visual components implementing the 9x9 Sudoku grid and fixed 3x3 positional candidate micro-grids (numpad layout 1-9 using empty text rather than visibility toggles to prevent reflow) within an `AspectRatioContainer` for dynamic portrait scaling. Sized to an enlarged ~707px board footprint with 75x75px cells (~6.5px screen edge margins) to maximize playable real estate and accessibility on mobile displays. Features crisp 1930s monochrome grid separation (4px white outer border, 6px white 3x3 block dividers, and 2px white inner cell dividers over dark cells) with clear visual differentiation between starting clues (bold, 75pt, pure white) and player inputs (67pt, dim gray `#a0a0a0`). Seamlessly binds to the core `SudokuBoard` model, handling cell selection toggling, peer highlighting, number matching, matching candidate note font enlargement (scaling from 19pt to 27pt bold in 24x24px slots), dynamic high-contrast text color inversion for highlighted cells, and real-time conflict error states.
- **Main Menu Screen (`game/scenes/main.tscn`, `game/scenes/main_menu.tscn`, `game/scripts/main_menu.gd`):** Application entry point embodying the 1930s rubber-hose aesthetic with a custom monochrome mascot character (`game/assets/icons/mascot_icon.jpg`). Features difficulty selection (Very Easy, Easy, Medium, Hard, Very Hard) that dynamically detects saved games via `SaveManager` to present "Resume" options with seamless timer continuation, a crisp 1930s monochrome horizontal separator line distinguishing gameplay actions from meta navigation, manages statistics navigation, opens a full-screen Credits and Studio Attributions modal (featuring a themed 'Got It!' dismissal button with a crisp 2px white outline for clear mobile affordance), and starts new puzzles seeded from `game/data/puzzles.json` with cleanly reset timer states.
- **Statistics Screen (`game/scenes/statistics_screen.tscn`, `game/scripts/statistics_screen.gd`):** Dedicated UI screen styled in 1930s monochrome presentation displaying player performance metrics (games started, games won, best times, and average times) across all five difficulty tiers (Very Easy, Easy, Medium, Hard, and Very Hard) on a single responsive page without scrollbars, featuring left-aligned metric values for clear column separation, dynamic layout scaling, and smooth back navigation to the main menu.
- **Input Controls & Numpad (`game/scenes/input_controls.tscn` & `game/scripts/input_controls.gd`):** Standalone 1930s-styled control panel wrapped in a `MarginContainer` (16px margins), featuring a cohesive segmented toggle control for Normal/Candidate modes (dynamic color inversion and merged borders), dedicated vertical separation, dynamically synchronized Undo button (disabled on empty history, enabled upon moves), 1-9 & Erase (X) numpad with balanced portrait button heights matching utility controls (80px), scaled 45px digit font size, and cheat-proof digit exhaustion dimming (`ThemeConstants.COLOR_NUMPAD_EXHAUSTED`), flat borderless Auto Candidate checkbox styling (`StyleBoxEmpty`) featuring prominent 2x scaled monochrome toggle switch icons (56x28), and full bi-directional (Cell-First and Number-First) input routing with enhanced visual matching and bolded candidate highlighting.
- **Gameplay Screen & Pause Overlay (`game/scenes/gameplay_screen.tscn`, `game/scripts/gameplay_screen.gd`, `game/scenes/pause_overlay.tscn`, `game/scripts/pause_overlay.gd`):** Top-level gameplay view organizing the responsive navigation header (< button with auto-saving, Difficulty label, active Timer, themed Pause button with 2px white outline, and Triple-dot Menu for Reset Puzzle & New Game), 9x9 Board, and Input Controls. Features a full-screen 1930s themed opaque pause overlay with mascot artwork to obscure the board, pause the timer, release the screen wake lock, and present a themed Resume button.
- **Victory Screen Overlay (`game/scenes/victory_overlay.tscn` & `game/scripts/victory_overlay.gd`):** Post-game celebration overlay featuring a full-screen responsive scaled layout styled with 1930s monochrome aesthetics (`res://resources/theme_1930s.tres`). Displays the scaled mascot artwork (`game/assets/icons/mascot_icon.jpg`), celebratory banner, and formatted completion time (`MM:SS`). Automatically stops the timer, records victory metrics in `StatsManager`, purges completed save files via `SaveManager`, and provides actions for "Play Again", "Main Menu", "Statistics", and "Admire Puzzle" (with an enlarged, horizontally centered "Victory Window" button below the game board covering the inactive input controls).
- **Platform & Rendering Configuration (`game/project.godot`):** Configured to use Godot's OpenGL Compatibility renderer (`gl_compatibility`) across desktop and mobile, along with forced V-Sync (`window/vsync/vsync_mode=1`). This resolves known Adreno 750 GPU driver conflicts and horizontal screen tearing on devices like the Samsung Galaxy S24 Ultra, while ensuring broad rendering stability across Android, Web (HTML5), and Desktop.
...
- **Session & Post-Game Controls:**
  ...
  - `Admire Puzzle`: Temporarily conceals the victory dialog card so the player can view their finished board, with an enlarged "Victory Window" button horizontally centered below the board covering inactive input controls to bring it back.
...
## Documentation Note
Note that any user-facing screenshots of the board should be updated to reflect the enlarged board footprint (~707px width with 75x75px cells) and scaled clue/input numbers (75pt and 67pt).
